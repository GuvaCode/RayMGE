...
type
  ...
  TTimer = record
    ...
    LastTick   : Double;
    OnTimer    : procedure;
    OnTimerEx  : procedure( Timer : PTimer );
    prev, next : PTimer;
end;

type
  PTimerManager = ^TTimerManager;
  TTimerManager = record
    Count : Integer;
    First : TTimer;
end;

function  timer_Add( OnTimer : Pointer; Interval : LongWord; UseSenderForCallback : Boolean = FALSE; UserData : Pointer = nil ) : PTimer;
procedure timer_Del( var Timer : PTimer );
...
var
  timersToKill  : Word = 0;
  aTimersToKill : array[ 0..1023 ] of PTimer;
  ...
  procedure ray_GetMem( out Mem : Pointer; Size : LongWord );
    begin
  if Size > 0 Then
    begin
      GetMem( Mem, Size );
...
begin
  ...
  ray_GetMem( Pointer( Result.next ), SizeOf( TTimer ) );
